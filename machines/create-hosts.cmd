...
@REM ******************* Be Cautious of deleting machiniens ******************************
...
rem @REM - Create a Swarm worker node for Docker local Registry to store images within the Firewall of the organizations
...
rem @REM - Create a worker node for rundeck for runbook automation
rem docker-machine create -d virtualbox --virtualbox-memory "2048" --swarm --swarm-discovery="consul://%dockerhost-keystore-ip%:8500" --engine-label nodename=dh-rundeck   --engine-opt="cluster-store=consul://%dockerhost-keystore-ip%:8500" --engine-opt="cluster-advertise=eth1:2376" dockerhost-rundeck

rem @REM - Create a Swarm worker node for hosting Applications 
rem docker-machine create -d virtualbox --virtualbox-memory "3072" --swarm --swarm-discovery="consul://%dockerhost-keystore-ip%:8500" --engine-label nodename=dh-worker --engine-opt="cluster-store=consul://%dockerhost-keystore-ip%:8500" --engine-opt="cluster-advertise=eth1:2376" dockerhost-worker

rem @REM - Create a Swarm worker node for hosting Applications 
rem @REM docker-machine create -d virtualbox --virtualbox-memory "2048" --swarm --swarm-discovery="consul://%dockerhost-keystore-ip%:8500" --engine-label="nodename=dh-ci" --engine-opt="cluster-store=consul://%dockerhost-keystore-ip%:8500" --engine-opt="cluster-advertise=eth1:2376" dockerhost-ci
...
@REM - Simply start the machine...


@REM - Stop all machines
docker-machine stop   dockerhost-rundeck dockerhost-worker dockerhost-registry  dockerhost-swarm-master dockerhost-keystore


@REM - Start all machines
docker-machine provision   dockerhost-keystore dockerhost-swarm-master   dockerhost-registry  dockerhost-worker dockerhost-rundeck
...
docker rm compose_worker_1  registry registry-data-container rundeck-data-container rundeck worker-data-container   jenkins-data-container--force

@REM - Docker Compose up - build from Dockerfile and spin up containers, networks, storage etc



docker-compose -f compose/docker-compose.yml  stop storage-registry  storage-rundeck  storage-worker storage-gitlab  storage-jenkins 
docker-compose -f compose/docker-compose.yml  stop registry  rundeck  worker gitlab jenkins
...
@REM docker exec -u 0 -it mycontainer bash   [ Login as root and execute]
